...
/**
 * Game Screen Class to generate the various screens used to play the game.
 * Instantiates all screen types and displays current screen.
 *
 * @author Ethan Alabaster, Adam Crook, Joe Dickinson, Sam Pearson, Tom Perry,
 *         Edward Poulter
 * @version 1.0
 */
public class GameScreen implements Screen {
	...
	/**
	 * Initialises the Game Screen, generates the world data and data for entities
	 * that exist upon it,
	 *
	...
	public GameScreen(PirateGame game, SaveLoader loadManager) {
		...
		/*
		 * //Random tornado Tornados = new ArrayList<>(); for (int i = 0; i < 5; i++) {
		 * pos = getRandomLocation(); //Add a tornado at the random coords
	...
	/**
	 * Makes this the current screen for the game. Generates the buttons to be able
	 * to interact with what screen is being displayed. Creates the escape menu and
	 * pause button
	 */
	@Override
	public void show() {
		...
		// Set the visibility of the tables. Particularly used when coming back from
		// options or skillTree
	...
	/**
	 * Checks for input and performs an action Applies to key "W" "A" "S" "D" "E"
	...
	public void handleInput(float dt) {
		...
		shopLabel = new Label("Press \"E\" to enter the Gold Shop",
	...
	public void update(float dt) {
		...
		tutorials.setPosition(camera.position.x - (tutorials.getWidth() / 2),
				camera.position.y - (tutorials.getHeight() / 2));
	...
	/**
	 * Renders the visual data for all objects Changes and renders new visual data
	 * for ships
	 *
	 * @param dt Delta time (elapsed time since last game tick)
	 */
	...
	/**
	 * Checks if the game is over i.e. goal reached (all colleges bar 0 are
	 * destroyed)
	 */
	public void gameOverCheck() {
		...
		else if (getCollege(CollegeMetadata.ANNELISTER).destroyed && getCollege(CollegeMetadata.CONSTANTINE).destroyed
				&& getCollege(CollegeMetadata.GOODRICKE).destroyed) {
	...
	/**
	 * Updates acceleration by a given percentage. Accessed by skill tree and power
	 * ups
	 *
